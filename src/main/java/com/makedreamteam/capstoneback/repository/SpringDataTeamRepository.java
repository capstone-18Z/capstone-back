...
public interface SpringDataTeamRepository extends JpaRepository<Team, UUID>  {
    ...
    @Query("SELECT mf.member,(mf.android*tf.android+mf.node*tf.node+mf.react*tf.react+mf.spring*tf.spring+mf.tdmax*tf.tdmax+mf.unity*tf.unity+mf.unreal*tf.unreal+mf.xcode*tf.xcode) from MemberFramework mf,TeamFramework  tf where tf.team.teamId = :teamId and mf.member.id in :memberId order by (mf.android*tf.android+mf.node*tf.node+mf.react*tf.react+mf.spring*tf.spring+mf.tdmax*tf.tdmax+mf.unity*tf.unity+mf.unreal*tf.unreal+mf.xcode*tf.xcode) DESC ")
