...
public interface MemberRepository extends JpaRepository<Member, String> {
    ...
    @Query("SELECT tf.team,(mf.android*tf.androidStudio+mf.node*tf.nodejs+mf.react*tf.react+mf.spring*tf.spring+mf.tdmax*tf.tdmax+mf.unity*tf.unity+mf.unreal*tf.unrealEngine+mf.xcode*tf.xcode) from MemberFramework mf,TeamFramework  tf where mf.member.id = :memberId and tf.team.teamId in :teamId order by (mf.android*tf.androidStudio+mf.node*tf.nodejs+mf.react*tf.react+mf.spring*tf.spring+mf.tdmax*tf.tdmax+mf.unity*tf.unity+mf.unreal*tf.unrealEngine+mf.xcode*tf.xcode) DESC ")
